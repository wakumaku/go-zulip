...
func TestFetchSingleMessage(t *testing.T) {
	...
	assert.False(t, resp.Message.IsMeMessage)
	assert.Equal(t, 27, resp.Message.RecipientID)
	assert.Equal(t, "**Don't** forget your towel!", resp.RawContent)
	assert.Len(t, resp.Message.Flags, 1)
	assert.Equal(t, "read", resp.Message.Flags[0])
	assert.False(t, resp.Message.DisplayRecipient.IsChannel)
	assert.Len(t, resp.Message.DisplayRecipient.Users, 3)
	...
	assert.False(t, resp.Message.DisplayRecipient.Users[0].IsMirrorDummy)
